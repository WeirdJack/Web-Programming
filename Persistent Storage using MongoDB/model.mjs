...
function fromBooksDb(collectionData) {
...
export default class Model {
  ...
  static async make(dbUrl) {
    ...
    try {
      ...
      const db = client.db(dbUrl.slice(dbUrl.lastIndexOf('/') + 1));
  ...
  async newCart(rawNameValues) {
    ...
    //@TODO
    const cartID = String(await this.shopping_cart.countDocuments() + Math.random());
    const ret = await this.shopping_cart.insertOne({"_lastModified": new Date(), "_id": cartID})
  ...
  async cartItem(rawNameValues) {
    ...
    //@TODO
    const cart_id = await this.shopping_cart.findOne({"_id": nameValues.cartId})
    if (cart_id !== null){

        await this.findBooks({"isbn": nameValues.sku})
        let myquery = { "_id": nameValues.cartId };
        const sku = nameValues.sku;
        let object = {};
        object[sku] = nameValues.nUnits;
        let newvalues = nameValues.nUnits !== 0? { $set:  object  } : { $unset: object};
        await this.shopping_cart.updateOne(myquery, newvalues, {upsert : true});
    } else {
      const msg = `no updates for cart ${JSON.stringify(nameValues.cartId)}`;
  ...
  async getCart(rawNameValues) {
    ...
    if (cart_id !== null){
      ...
      let k = this.shopping_cart.find({"_id": nameValues.cartId})
      const dbData = await k.toArray();
      const l = dbData.map(e => fromBooksDb(e));
      return l[0];
  ...
  async addBook(rawNameValues) {
    ...
    //@TODO
    let query = { "isbn": nameValues.isbn };
    const set = Object.assign({"_lastModified": new Date()}, nameValues);
    await this.book_catalog.updateOne(query, { $set: set }, {upsert : true});
    let k = this.book_catalog.find()
    const dbData = await k.toArray();
    const l = dbData.map(e => fromBooksDb(e));
    return l[l.length - 1].isbn;
  }
  ...
  async findBooks(rawNameValues) {
    ...
    //@TODO
    if (nameValues.isbn !== undefined){
      const bk_sku = await this.book_catalog.findOne({"isbn": nameValues.isbn});
      if (bk_sku !== null){
        let isbnSearch = fromBooksDb(await this.book_catalog.findOne({"isbn": nameValues.isbn}));
        return Array.of(isbnSearch);
      } else {
    ...
    dbData.map((e,i) => {
      let title = (dbData[i].title).match(regex);
      let author = (dbData[i].authors.toString().toLowerCase()).match(regex);
      (title.includes(nameValues.authorsTitleSearch) || author.includes(nameValues.authorsTitleSearch))? result.push(fromBooksDb(e)) : e;
    })
    return result.slice(index, index + count);
  }
...
const OBJECTS = [ 'book_catalog', 'shopping_cart']